...
import chart.model.LineSet;
import chart.renderer.AxisRenderer;
import chart.view.BarChartView;
import chart.view.BaseBarChartView;
import chart.view.BaseStackBarChartView;
import chart.view.ChartView;
import chart.view.HorizontalBarChartView;
import chart.view.HorizontalStackBarChartView;
import chart.view.LineChartView;
import chart.view.StackBarChartView;
...
public class SandboxFragment extends Fragment {
	...
	private static ChartView buildBarChart(BaseBarChartView chart) {
		...
		if (mHasBarBackground) chart.setBarBackgroundColor(mBarBackgroundColorId);
	...
	private static void generateChart() {
		...
		switch (mChartId) {
			case R.id.sandbox_chart_line:
				buildChart(
						  buildLineChart((LineChartView) mCurrFragment.getView().findViewById(mChartId)));
				break;
			case R.id.sandbox_chart_bar:
				buildChart(
						  buildBarChart((BarChartView) mCurrFragment.getView().findViewById(mChartId)));
				break;
			case R.id.sandbox_chart_horbar:
				buildChart(buildBarChart(
						  (HorizontalBarChartView) mCurrFragment.getView().findViewById(mChartId)));
				break;
			case R.id.sandbox_chart_stacked:
				buildChart(buildStackedChart(
						  (StackBarChartView) mCurrFragment.getView().findViewById(mChartId)));
				break;
			case R.id.sandbox_chart_horstacked:
				buildChart(buildStackedChart(
						  (HorizontalStackBarChartView) mCurrFragment.getView().findViewById(mChartId)));
				break;
			default:
				buildChart(
						  buildLineChart((LineChartView) mCurrFragment.getView().findViewById(mChartId)));
	...
	private static String buildAnimationCode() {
		...
		else code.append("Animation anim = new Animation();\n");
		...
		if (mAlpha != -1) code.append("anim.setAlpha(").append(mAlpha).append(");\n");
		...
		if (mOverlapOrder[0] != 0 || mOverlapFactor != 1) {
			...
			order += "}";
			code.append("anim.setOverlap(")
					  .append(mOverlapFactor)
					  .append(", ")
					  .append(order)
					  .append(");\n");
		}

		if (mStartX != -1 && mStartY != 0) code.append("anim.setStartPoint(")
				  .append(mStartX)
				  .append(", ")
				  .append(mStartY)
				  .append(");\n");
	...
	private String buildLineCode() {
		...
		if (mIsLineDashed) code.append("dataset.setDashed(true);\n");

		if (mIsLineSmooth) code.append("dataset.setSmooth(true);\n");

		code.append("dataset.setThickness(Tools.fromDpToPx(" + mLineThickness + "));\n");
		if (mLineColorId != DEFAULT_COLOR) code.append("dataset.setColor(Color.parseColor('#")
				  .append(Integer.toHexString(mLineColorId).substring(2))
				  .append("'));\n");

		if (mPointsSizeId != -1)
			code.append("dataset.setDotsRadius(Tools.fromDpToPx(").append(mPointsSize).append("));\n");
		if (mPointColorId != DEFAULT_COLOR) code.append("dataset.setDotsColor(Color.parseColor('#")
				  .append(Integer.toHexString(mPointColorId).substring(2))
				  .append("'));\n");
	...
	private String buildBarCode() {
		...
		if (mBarColorId != DEFAULT_COLOR) code.append("dataset.setColor(Color.parseColor('#")
				  .append(Integer.toHexString(mBarColorId).substring(2))
				  .append("'));\n");
		if (mBarSpacing != 0) code.append("chart.setBarSpacing(").append(mBarSpacing).append(");\n");
		if (mHasBarBackground) code.append("chart.setBarBackgroundColor(Color.parseColor('#")
				  .append(Integer.toHexString(mBarBackgroundColorId).substring(2))
				  .append("'));\n");
	...
	private String buildStackedCode() {
		...
		if (mBarColorId != DEFAULT_COLOR) code.append("dataset.setColor(Color.parseColor('# ")
				  ...
				  .append("'));\n");
		if (mBarSpacing != 0) code.append("chart.setBarSpacing(").append(mBarSpacing).append(");\n");
		if (mHasBarBackground) code.append("chart.setBarBackground(Color.parseColor('#")
				  .append(Integer.toHexString(mBarBackgroundColorId).substring(2))
				  .append("'));\n");
	...
	private String buildCode() {
		...
		switch (mChartId) {
			case R.id.sandbox_chart_line:
				code.append(
						  "LineChartView chart = (LineChartView) <layout>.findViewById(<chart_id>);\n");
			...
			case R.id.sandbox_chart_horbar:
				code.append(
						  "HorizontalBarChartView chart = (HorizontalBarChartView) <layout>.findViewById(<chart_id>);\n");
				code.append(buildBarCode());
				break;
			case R.id.sandbox_chart_stacked:
				code.append(
						  "StackBarChartView chart = (StackBarChartView) <layout>.findViewById(<chart_id>);\n");
				code.append(buildStackedCode());
				break;
			case R.id.sandbox_chart_horstacked:
				code.append(
						  "HorizontalStackChartView chart = (HorizontalStackChartView) <layout>.findViewById(<chart_id>);\n");
				code.append(buildStackedCode());
				break;
			default:
		}
		...
		if ((mHasXAxis || mHasYAxis) && mAxisColorId != DEFAULT_COLOR)
			code.append("chart.setAxisColor(Color.parseColor('#")
					  .append(Integer.toHexString(mAxisColorId).substring(2))
					  .append("'));\n");
		...
		if ((mXLabelPosition != AxisRenderer.LabelPosition.NONE ||
				  mYLabelPosition != AxisRenderer.LabelPosition.NONE) && mLabelColorId != DEFAULT_COLOR)
			code.append("chart.setLabelsColor(Color.parseColor('#")
					  .append(Integer.toHexString(mLabelColorId).substring(2))
					  .append("'));\n");
		if (mGridType != null) {
			code.append("// Paint object used to draw Grid\n" +
					  "Paint gridPaint = new Paint();\n" +
					  "gridPaint.setColor(Color.parseColor('#")
					  .append(Integer.toHexString(mGridColorId).substring(2))
					  .append("'));\n")
					  .append("gridPaint.setStyle(Paint.Style.STROKE);\n")
					  .append("gridPaint.setAntiAlias(true);\n")
					  .append("gridPaint.setStrokeWidth(Tools.fromDpToPx(")
					  .append(mGridThickness)
					  .append("));\n");
			if (mIsGridDashed) code.append("gridPaint.setPathEffect(new DashPathEffect(")
					  .append(mGridDashType)
					  .append(", 0));\n");
		...
		if (mLabelFormat != "") code.append("chart.setLabelsFormat(new DecimalFormat('#'+")
				  .append(mLabelFormat)
				  .append("));\n");
	...
	public static class SandboxPagerAdapter extends FragmentStatePagerAdapter {
		...
		public Fragment getItem(int position) {
			...
			switch (position) {
				...
				case 3:
					if (mChartId == R.id.sandbox_chart_line) return new SandBoxLineFragment();
					else return new SandBoxBarFragment();
				case 4:
					return new SandBoxAnimationFragment();
				case 5:
					return new SandBoxPlayFragment();
				default:
					return new SandBoxChartFragment();
			}
		}
	...
	public abstract static class BaseSandBoxFragment extends Fragment {
		...
		static int swapState(View parentView, int oldId, int newId, boolean mandatoryAttr) {
			...
			if (oldId == newId && !mandatoryAttr) {
				parentView.findViewById(newId)
						  .setSelected(!parentView.findViewById(newId).isSelected());
				newId = -1;
	...
	public static class SandBoxAxisFragment extends BaseSandBoxFragment {
		...
		public View onCreateView(LayoutInflater inflater, ViewGroup container,
			...
			final EditText editText =
					  (EditText) mLayout.findViewById(R.id.sandbox_axis_label_format_value);
		...
		protected void onStateRecover() {
			...
			if (mLabelXId != -1) mLayout.findViewById(mLabelXId).setSelected(true);
			if (mAxisXId != -1) mLayout.findViewById(mAxisXId).setSelected(true);
			if (mLabelYId != -1) mLayout.findViewById(mLabelYId).setSelected(true);
			if (mAxisYId != -1) mLayout.findViewById(mAxisYId).setSelected(true);
			((GradientDrawable) (mLayout.findViewById(
					  R.id.sandbox_axis_color)).getBackground()).setColor(mAxisColorId);
			((GradientDrawable) (mLayout.findViewById(
					  R.id.sandbox_label_color)).getBackground()).setColor(mLabelColorId);
		}
		...
		protected void onStateChange(int id) {
			...
			switch (id) {
				...
				case R.id.sandbox_axis_y_outside:
					if (id == mLabelYId) mYLabelPosition = AxisRenderer.LabelPosition.NONE;
					else mYLabelPosition = AxisRenderer.LabelPosition.OUTSIDE;
					mLabelYId = swapState(mLayout, mLabelYId, id, false);
					break;
				case R.id.sandbox_axis_y_inside:
					if (id == mLabelYId) mYLabelPosition = AxisRenderer.LabelPosition.NONE;
					else mYLabelPosition = AxisRenderer.LabelPosition.INSIDE;
					mLabelYId = swapState(mLayout, mLabelYId, id, false);
					break;
				...
				case R.id.sandbox_axis_x_inside:
					if (id == mLabelXId) mXLabelPosition = AxisRenderer.LabelPosition.NONE;
					else mXLabelPosition = AxisRenderer.LabelPosition.INSIDE;
					mLabelXId = swapState(mLayout, mLabelXId, id, false);
					break;
				case R.id.sandbox_axis_x_outside:
					if (id == mLabelXId) mXLabelPosition = AxisRenderer.LabelPosition.NONE;
					else mXLabelPosition = AxisRenderer.LabelPosition.OUTSIDE;
					mLabelXId = swapState(mLayout, mLabelXId, id, false);
					break;
	...
	public static class SandBoxGridFragment extends BaseSandBoxFragment {
		...
		protected void onStateRecover() {
			...
			if (mGridTypeId != -1) mLayout.findViewById(mGridTypeId).setSelected(true);
				...
				case R.id.sandbox_grid_color:
					...
					mDialogBuilder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int id) {
							...
							((GradientDrawable) (mLayout.findViewById(
									  R.id.sandbox_grid_color)).getBackground()).setColor(mGridColorId);
						}
	...
	public static class SandBoxLineFragment extends BaseSandBoxFragment {
		...
		protected void onStateRecover() {
			...
			if (mLineTypeId != -1) mLayout.findViewById(mLineTypeId).setSelected(true);
			mLayout.findViewById(mLineThicknessTypeId).setSelected(true);
			mLayout.findViewById(mLineThicknessId).setSelected(true);
			((GradientDrawable) (mLayout.findViewById(
					  R.id.sandbox_line_color)).getBackground()).setColor(mLineColorId);
			if (mPointsSizeId != -1) mLayout.findViewById(mPointsSizeId).setSelected(true);
			((GradientDrawable) (mLayout.findViewById(
					  R.id.sandbox_point_color)).getBackground()).setColor(mPointColorId);
		}
	...
	public static class SandBoxBarFragment extends BaseSandBoxFragment {
		...
		protected void onStateRecover() {
			...
			((GradientDrawable) (mLayout.findViewById(
					  R.id.sandbox_bar_color)).getBackground()).setColor(mBarColorId);
			mLayout.findViewById(mBarSpacingId).setSelected(true);
			((GradientDrawable) (mLayout.findViewById(
					  R.id.sandbox_bar_background_color)).getBackground()).setColor(
					  mBarBackgroundColorId);
			if (mBarBackgroundId != -1) mLayout.findViewById(mBarSpacingId).setSelected(true);
		}
		...
		protected void onStateChange(int id) {
			...
			switch (id) {
				...
				case R.id.sandbox_bar_background_color:
					final View backgroundColorLayout = getActivity().getLayoutInflater()
							  .inflate(R.layout.color_picker, mViewGroup, false);
					...
					mDialogBuilder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int id) {
							...
							ColorPicker picker =
									  (ColorPicker) backgroundColorLayout.findViewById(R.id.picker);
	...
	public static class SandBoxAnimationFragment extends BaseSandBoxFragment {
		...
		public View onCreateView(LayoutInflater inflater, ViewGroup container,
			...
			ArrayAdapter<CharSequence> adapterDuration =
					  ArrayAdapter.createFromResource(this.getActivity(), R.array.duration,
								 R.layout.spinner_item);
			adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		...
		protected void onStateRecover() {
			...
			if (mAlphaId != -1) mLayout.findViewById(mAlphaId).setSelected(true);
		}
